...
export function Botao({
  ...
  const larguraBtn = larguraCompleta ? 'w-full' : 'w-auto'
  const varsLargura = {
    primario: `bg-yellow-500 hover:bg-yellow-600 text-navy-900 ${larguraBtn}`,
    secundario: `bg-white hover:bg-gray-100 text-navy-900 border border-gray-300 ${larguraBtn}`,
    contorno: `bg-transparent hover:bg-gray-100 text-navy-900 border-2 border-navy-500 ${larguraBtn}`,
  }
  ...
  ({ rotulo, erro, className = '', ...props }, ref) => {
    return (
      <div>
        ...
        <input
          ref={ref}
          className={`w-full px-4 py-3 border rounded-lg outline-none focus:ring-2 focus:ring-yellow-500 focus:border-transparent ${erro ? 'border-red-500' : 'border-gray-300'
            } ${className}`}
          {...props}
        />
  ...
  return (
    <Card hover>
      <div className="hero-sm p-8 text-center">
        <div className="text-6xl mb-4">{maquina.img_url ? <img src={maquina.img_url} alt={maquina.nome} /> : '🚜'}</div>
        <h3 className="text-xl font-bold text-navy-900">{maquina.nome}</h3>
      </div>
      <div className="p-6">
        ...
        <div className="space-y-2 mb-4 texto">
          <div className="flex-gap">
            ...
            <span>{maquina.proprietario?.nome || "Carregando..."}</span>
          </div>
          <div className="flex-gap">
        ...
        <div className="flex-between mb-4">
          <span className="text-2xl font-bold text-navy-900">R$ {maquina.preco_diaria}/hora</span>
        </div>
        <a href={`/maquina/${maquina.id}`} className="block w-full">
