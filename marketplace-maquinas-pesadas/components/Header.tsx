...
import { Menu, X, User, LogIn, Truck } from 'lucide-react'
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        {isMenuOpen && (
          <nav className="md:hidden pb-4 space-y-2">
            <Link href="/busca" className="nav-link-mobile" onClick={() => setIsMenuOpen(false)}>Buscar Máquinas</Link>
            <Link href="/como-funciona" className="nav-link-mobile" onClick={() => setIsMenuOpen(false)}>Como Funciona</Link>
            <Link href="/sobre" className="nav-link-mobile" onClick={() => setIsMenuOpen(false)}>Sobre</Link>
            <Link href="/login" className="nav-link-mobile flex items-center space-x-2" onClick={() => setIsMenuOpen(false)}>
              <LogIn className="w-4 h-4" />
              <span>Entrar</span>
            </Link>
            <Link href="/cadastro" className="nav-btn-mobile" onClick={() => setIsMenuOpen(false)}>Cadastrar</Link>
          </nav>
        )}
      </div>
    </header>
