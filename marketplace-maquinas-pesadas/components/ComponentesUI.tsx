...
export function Botao({
  ...
  const base = 'font-semibold px-6 py-3 rounded-lg transition-colors disabled:opacity-50 disabled:cursor-not-allowed'
  const vars = {
  ...
  return (
    <button
      className={`${base} ${vars[variante]} ${largura} ${className}`}
      {...props}
    >
      {conteudo}
    </button>
...
// Campo de Entrada
interface PropsCampoEntrada extends InputHTMLAttributes<HTMLInputElement> {
  rotulo?: string
  erro?: string
}

export const CampoEntrada = forwardRef<HTMLInputElement, PropsCampoEntrada>(
  ({ rotulo, erro, className = '', ...props }, ref) => {
    return (
      <div>
        ...
        <input
          ref={ref}
          className={`w-full px-4 py-3 border rounded-lg outline-none focus:ring-2 focus:ring-yellow-500 focus:border-transparent ${erro ? 'border-red-500' : 'border-gray-300'} ${className}`}
          {...props}
        />
...
export function Card({ filhos, children, className = '', hover = false }: PropsCard) {
...
export function Avaliacao({ nota, totalAvaliacoes, tamanho = 'medio' }: PropsAvaliacao) {
  const tamanhos = {
    pequeno: 'w-4 h-4',
    medio: 'w-5 h-5',
    grande: 'w-6 h-6',
...
interface PropsBarraBusca {
  ...
  aoMudarLocalizacao: (valor: string) => void
  aoBuscar?: () => void
...
// Card de Máquina
interface PropsCardMaquina {
  maquina: Maquina
}

export function CardMaquina({ maquina }: PropsCardMaquina) {
  return (
    <Card hover>
      ...
      <div className="p-6">
        ...
        <a
          ...
          className="block w-full btn-amarelo-sm text-center py-3"
        >
          Ver Detalhes
        </a>
      </div>
